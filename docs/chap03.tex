...
\begin{figure}[htbp]
    ...
    \caption{大小模型协同研究框架总览：输入轨迹经TSPM与GA-LLM双分支编码后，通过对齐与协同训练提升语义分支质量，最终Top-$K$由GA-LLM推理输出。}
    \label{fig:c3-framework}
\end{figure}

如图\ref{fig:c3-framework}所示，框架完整展示了从轨迹输入、双分支特征学习到GA-LLM输出的端到端信息流。该结构说明本文并非简单模型拼接，而是通过映射与联合优化提升语义分支对结构先验的吸收能力。该设计对应第\ref{chap:exp}章RQ1与RQ4，用于验证双路线互补带来的整体收益。
...
TSPM分支首先构建可训练的POI初始表示，以保证后续时间分槽与动态图计算有稳定输入。本文采用“关系旋转表示 + 局部拓扑保持”的组合初始化：前者建模转移关系方向性，后者保持近邻几何结构。
\begin{equation}
...
该初始化对应 TSPM 论文中的核心做法\cite{liu2025tspm}，其作用是减少“随机初始化导致的早期训练震荡”，并为后续 TSDG 的边权学习提供更平滑的优化起点。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.43\textwidth}
        \centering
        \includegraphics[height=0.20\textheight,keepaspectratio]{Time_enhanced_left.pdf}
        \caption{传统连续序列}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.53\textwidth}
        \centering
        \includegraphics[height=0.20\textheight,keepaspectratio]{Time_enhanced_right.pdf}
        \caption{时间增强序列}
    \end{subfigure}
...
推理阶段采用 beam search 直接生成候选并输出 Top-$K$。本文保持与投稿论文一致的单路生成口径：\bluebf{不引入候选约束解码、不引入在线重排序、不引入TSPM推理打分}。这一定义确保方法叙述与实验实现完全一致\cite{liu2026gallm}。
...
在线部署时，建议保持“GA-LLM直推”的主链路，并通过 Beam size、Top-$K$ 与提示长度控制时延。当出现地理字段缺失、模板解析失败或模型超时时，可回退到基础语义模板与规则过滤路径以保证服务可用性。
...
\begin{table}[htbp]
    \cThreeTblStyle
    \caption{代表方法能力边界对照（“\ding{51}”表示具备主能力）}
