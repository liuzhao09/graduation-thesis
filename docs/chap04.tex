...
\newcommand{\tblstyle}{\centering\small\setlength{\tabcolsep}{4.0pt}\renewcommand{\arraystretch}{1.16}}
...
\section{实验设置}
\subsection{数据集与预处理}
实验采用 Gowalla、Foursquare、NYC、TKY、CA 等公开数据集，均包含用户ID、POI ID、时间戳、经纬度与类别信息。预处理遵循常见Next POI设定：过滤极低频用户与POI、按时间排序构造轨迹，并保证训练/验证/测试在时间上严格先后，避免信息泄露\cite{LLM4POI,GETNext,STHGCN}。
...
\section{线上评估方案与风险控制建议}
\subsection{离线到在线的指标映射}
离线指标（Acc/MRR/NDCG）与在线指标（点击率、转化率、停留时长）并非一一对应。建议采用“离线筛选 + 小流量在线验证”两阶段评估：先在离线确保准确率与稳定性，再在在线验证用户行为收益，减少直接上线的风险。

\subsection{A/B测试分层建议}
建议按用户活跃度、城市区域与时间段分层开展A/B测试，避免总体均值掩盖局部退化。对于高价值场景（晚高峰、跨区迁移），单独监控失败率与延迟指标；对于低活跃用户，重点观察冷启动收益与召回覆盖变化。

\subsection{风险控制与回退策略}
协同系统上线需预置风险控制机制：当在线延迟超阈值、候选置信度异常或地理字段缺失时，自动回退到轻量级策略（如TSPM-only）保障服务稳定；同时保留灰度开关，支持按城市或人群分批回滚，降低全局风险。

\section{复现性、可靠性与威胁分析}
\subsection{复现性说明}
