...
为验证两条路线的互补关系，本文对TSPM与GA-LLM的错误类型进行并列分析。结果显示，二者在以下两类错误上具有明显分工：
\begin{enumerate}
    \item \textbf{空间远跳错误}：由语义偏置引起的地理不合理预测；
    \item \textbf{历史重复错误}：由局部历史过拟合导致的高频点重复预测。
\end{enumerate}
这表明小模型提供的结构约束与大模型提供的语义推理形成了互补机制，而非同质重复。
...
\end{enumerate}
分析显示，GA-LLM在引入GCIM与PAM后对前两类错误改善最明显，说明地理约束与结构先验对语义误导具有有效校正作用。

\subsubsection{案例分析}
选取典型样本观察可见：当用户轨迹中出现“办公区$\rightarrow$商圈”类跨区域迁移时，纯序列模型倾向保守预测，纯LLM模型可能给出语义合理但地理偏远的点；加入GCIM与PAM后的GA-LLM更常给出“语义可解释且地理可达”的候选。该现象与本文方法目标高度一致。

\subsubsection{协同收益边界}
需要指出，不同路线并非在所有样本上都同等占优。对于轨迹极短且候选稀疏的样本，模型仍可能受限于信息不足。该结果提示后续工作可结合外部上下文（天气、节假日、交通状态）进一步提升极端场景表现。
...
\subsection{轨迹长度与解码规模的可扩展性补充}
为进一步验证模型在不同规模输入下的可扩展性，本文从“轨迹长度变化”和“生成解码规模变化”两个维度进行补充观察。结果显示，随着轨迹长度增加，推理时延与显存占用呈稳定增长趋势；随着 beam size 与 Top-$K$ 增大，时延增长更明显，符合“生成解码主导成本”的复杂度预期。该现象与第\ref{chap:method}章复杂度分析一致，说明本文方法在规模扩展时具有可预测的资源行为，便于工程部署中的容量规划。
...
本文按轨迹长度、地理跨度与POI流行度三类维度对测试样本分群。整体趋势见表\ref{tab:c4-segment-summary}。结果表明，GA-LLM路线在短轨迹和长尾POI场景的相对收益更明显，说明语义分支在信息不足时的补偿作用与结构先验注入在长尾场景的约束作用同时发挥效果。

\begin{table}[htbp]
    \tblstyle
    \caption{分群评估趋势总结（GA-LLM路线）}
...
围绕时间槽数量 $Z$、LoRA 秩 $r$ 与解码规模（beam size / Top-$K$）的敏感性现象总结如表\ref{tab:c4-hparam-sensitivity}。该结果用于界定模型的可调节边界，避免在工程部署中只追求单指标最优而忽视稳定性。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{P{0.20\textwidth}P{0.35\textwidth}P{0.35\textwidth}}
        ...
        Beam size / Top-$K$ & 生成覆盖不足导致命中受限 & 推理时延上升、收益边际递减 \\
        \bottomrule
    \end{tabular}
\end{table}
...
在“居住区$\rightarrow$办公区$\rightarrow$餐饮区”的通勤样本中，引入GCIM与PAM的GA-LLM能够在午间时段给出地理可达且语义匹配的餐饮候选，而纯序列模型更易重复办公区附近高频点；在“周末商圈漫游”样本中，纯LLM模型偶尔产生语义相关但距离偏远的候选，GA-LLM路线通常能保持更好的空间可达性。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{P{0.14\textwidth}P{0.22\textwidth}P{0.28\textwidth}P{0.28\textwidth}}
        ...
        CA & 广域稀疏、长距离迁移更多 & 冷启动与远跳误判风险高 & GCIM约束地理可达性，降低远距离语义幻觉 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{P{0.20\textwidth}P{0.22\textwidth}P{0.24\textwidth}P{0.24\textwidth}}
        ...
        \midrule
        地理远跳误判 & 预测点与历史活动区距离突增 & GCIM / 候选过滤规则 & 增强测地约束权重与地理可达性过滤 \\
