...
近两年生成式推荐进一步改变了建模方式，即将“候选打分”转化为“下一条交互标识生成”。从 P5~\cite{p5recsys2022} 统一范式到针对ID构造与约束生成的 CID~\cite{cid2023}、GenRec~\cite{genrec2023}、Tiger~\cite{tiger2023neurips}，再到强调协同语义对齐的 IDGenRec~\cite{idgenrec2024sigir}，研究重点逐步从“能生成”转向“生成可控、可对齐、可落地”。这一趋势与Next POI任务高度相关，因为下一地点预测本质上也是在强约束空间中生成合法且可达的目标标识。
...
\begin{enumerate}
    ...
    \item \textbf{小模型机制设计}：围绕时间异质性与迁移方向性，设计时间增强序列动态图（Time-enhanced Sequence-based Dynamic Graph, TSDG）与双向转移建模，并通过动态图权重增强复杂场景区分能力；
    \item \textbf{大模型增强设计}：围绕地理连续性与转移先验缺失，设计地理坐标注入模块（Geographic Coordinate Injection Module, GCIM）与POI对齐模块（POI Alignment Module, PAM），将结构先验以可训练方式注入LLM；
...
围绕“标识构造-语义对齐-受约束生成”的技术主线，本文将相关进展进一步映射到自身问题：在构造层，借鉴 CID~\cite{cid2023} 与 Tiger~\cite{tiger2023neurips} 的语义ID思路提升POI标识表达能力；在对齐层，参考 CLLM4Rec~\cite{cllm4rec2024www}、A-LLMRec~\cite{allmrec2024kdd} 与 IDGenRec~\cite{idgenrec2024sigir} 的协同对齐范式；在生成层，结合 TALLRec~\cite{tallrec2023} 与 GenRec~\cite{genrec2023} 的受约束生成实践，服务于Next POI的可达性与一致性目标。
