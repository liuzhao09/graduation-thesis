...
对Next POI任务而言，显式推理的价值主要在三点：其一，可将“距离可达性、时段约束、历史路径方向”显式纳入决策；其二，可将结构先验作为推理证据而非黑盒隐变量；其三，可在错误分析中定位到底是语义误判还是约束违背。本文借鉴其“可控与可诊断”思想，但不引入候选过滤链、在线约束校验或自反思中间输出，而是通过 GCIM 与 PAM 在嵌入空间施加隐式一致性约束。
