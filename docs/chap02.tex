...
{\heiti\addCJKfontfeatures{AutoFakeBold=2}局限与启示：}序列模型对局部行为刻画精细，但在时间异质性显式建模和跨用户高阶迁移利用上仍不足，提示本文需引入“时间分段建模 + 结构化转移先验”。

{\heiti\addCJKfontfeatures{AutoFakeBold=2}进一步讨论：}序列路线在“远距离跳转”场景下往往不稳定。原因很直接：当模型只看短期历史时，通常会把高频近邻点排在前面，因此在跨区域通勤或目的地明确的出行中容易偏离真实目标。换言之，仅靠加深序列编码器，通常只能提高局部拟合能力，无法自动学到“可达性约束”和“全局转移关系”；因此仍需显式引入图结构或地理约束。
...
对本研究问题而言，上述进展提供了三点直接启示：{\heiti\addCJKfontfeatures{AutoFakeBold=2}其一：}仅依赖自然语言提示难以稳定承载结构迁移先验；{\heiti\addCJKfontfeatures{AutoFakeBold=2}其二：}表示对齐应覆盖“ID语义-行为图结构-上下文意图”三类信息；{\heiti\addCJKfontfeatures{AutoFakeBold=2}其三：}方法设计需同步考虑推理效率与部署可控性，避免因复杂推理链带来时延与稳定性波动。这些认识为后文方法章节的模块化设计提供了相关工作层面的依据。
...
\subsubsection{ID构造、语义对齐与受约束生成}
近期工作可概括为“ID构造-语义对齐-受约束生成”三阶段。{\heiti\addCJKfontfeatures{AutoFakeBold=2}其一：}在ID构造层，P5~\cite{p5recsys2022} 将推荐任务统一为语言生成范式，CID~\cite{cid2023} 进一步讨论了不同Item ID编码方式对可学习性的影响。{\heiti\addCJKfontfeatures{AutoFakeBold=2}其二：}在语义对齐层，CLLM4Rec~\cite{cllm4rec2024www}、A-LLMRec~\cite{allmrec2024kdd} 与 IDGenRec~\cite{idgenrec2024sigir} 通过协同信号对齐缓解“ID空间-语言空间”错位。{\heiti\addCJKfontfeatures{AutoFakeBold=2}其三：}在生成阶段，TALLRec~\cite{tallrec2023}、GenRec~\cite{genrec2023} 与 Tiger~\cite{tiger2023neurips} 强调受约束或可校验生成，以降低非法ID输出与语义漂移。
...
{\heiti\addCJKfontfeatures{AutoFakeBold=2}局限与启示：}现有LLM方案普遍面临坐标语义稀疏、空间连续性不足和转移先验注入弱的问题，说明Next POI场景需要“结构先验 + 语义推理”的协同机制，而非纯文本提示。

\subsubsection{小结性对比}
为更清晰地展示三类方法的能力边界，表\ref{tab:c2-compare}给出总结性比较。
\begin{table}[htbp]
    \cTwoTblStyle
    \bitablecaption{序列、图与LLM路线能力边界对比}{Capability boundary comparison among sequential, graph-based, and LLM-based routes.}
...
相较于第2.2.2节对Next POI专项工作的梳理，本节更关注生成式推荐的一般方法演化及其对本任务的启示。随着生成式推荐的发展，研究重点已从“把推荐写成文本生成”逐步转向“提升生成过程的可控性与可诊断性”：在复杂推荐任务中，仅靠隐式注意力通常难以稳定执行约束推断，因此显式推理链、检索增强与结构对齐成为常见改进方向。

\subsubsection{从指令调优到可控生成}
在上述转向下，生成式推荐大体经历三步：{\heiti\addCJKfontfeatures{AutoFakeBold=2}第一步，}把推荐任务指令化并做参数高效微调；{\heiti\addCJKfontfeatures{AutoFakeBold=2}第二步，}把召回、排序、解释统一到生成框架；{\heiti\addCJKfontfeatures{AutoFakeBold=2}第三步，}引入检索、对齐、约束等机制，提升可控性与稳定性\cite{li2023instructrec,hou2024recgpt,tallrec2023,bao2023chatrec,LLMRec,CoLLM,CoRAL,ReLLa,LLaRA,lin2024llmrank,fan2024memo,sun2024promptrec,wu2024ragrec}。这一演化说明：高质量推荐不只看“能否生成”，还要看“能否稳定且可诊断地生成”。

\subsubsection{显式推理范式进展}
与“直接生成答案”不同，显式推理范式强调中间决策轨迹，例如候选过滤、约束检查、理由归纳与自我校验。代表性工作显示，将可追溯推理过程纳入推荐推断有助于提升一致性与可解释性\cite{qu2024onerec,qu2024onerecthink,wu2024thinkrec,xu2024hybridrec}。这类方法的共同点是：模型不只输出最终推荐，还输出可追溯的中间依据，从而降低“结果可用但难解释”的工程风险。

对Next POI任务而言，显式推理的价值主要在三点：{\heiti\addCJKfontfeatures{AutoFakeBold=2}其一：}可将“距离可达性、时段约束、历史路径方向”显式纳入决策；{\heiti\addCJKfontfeatures{AutoFakeBold=2}其二：}可将结构先验作为推理证据而非黑盒隐变量；{\heiti\addCJKfontfeatures{AutoFakeBold=2}其三：}可在错误分析中定位到底是语义误判还是约束违背。本文借鉴其“可控与可诊断”思想，但不引入候选过滤链、在线约束校验或自反思中间输出，而是通过 GCIM 与 PAM 在嵌入空间施加隐式一致性约束。

\subsubsection{与本文方法的关系}
本文不直接复制通用 CoT 模板，而是采用“结构对齐 + 协同训练”的技术路线。原因在于：Next POI场景具有强时空约束与低时延要求，若直接引入冗长推理链，可能带来明显时延与鲁棒性波动。相比之下，本文将约束能力压缩到“可学习对齐模块 + 统一生成目标”，并保持推理阶段 GA-LLM 单路输出，不采用 constrained decoding、logit mask 或在线重排序。

\subsection{中文研究脉络与本土场景启示}
中文学术界在推荐系统、序列推荐和图推荐方面已形成较系统研究脉络\cite{liu2024cn1,zhang2023cn2,wang2023cn3,li2024cn4}。在时空行为建模与位置推荐方面，研究重点逐步转向“地理约束 + 时间规律”的联合建模\cite{zhao2022cn5,he2021cn6,sun2022cn7,yang2023cn8,zheng2024cn14}。在大模型推荐与生成式推荐方面，近年的讨论集中于可解释性、RAG与工程落地\cite{chen2024cn9,gao2024cn10,huang2024cn11,xu2024cn12,qian2024cn15}，并与城市时空智能应用形成交叉\cite{liu2023cn13}。

这些中文研究对本文有两点直接启示。第一，真实业务中“结果可解释”与“工程可用”通常与“离线最优”同等重要，方法设计不能只追求指标提升；本文在第\ref{chap:exp}章RQ5对效率与可部署性进行实证验证。第二，国内LBS与本地生活场景具备高密度、强约束、强实时特征，要求推荐模型在地理一致性与时延之间做更细粒度权衡。本文采用协同框架而非单一路线，正是基于上述工程现实做出的方法选择。

\subsection{扩展文献归纳}
为完整刻画本文研究语境，本文参考了经典协同过滤与排序学习文献\cite{koren2009matrix,rendle2009bpr,FPMC,he2017ncf}，以及序列推荐主线工作\cite{hidasi2016gru4rec,kang2018sasrec,sun2019bert4rec,zhang2023sequentialsurvey}。在图推荐与自监督学习方面，重点参考了轻量图卷积、知识图增强与图对比学习工作\cite{he2020lightgcn,wang2019kgat,zhou2020sgl,lin2022simgcl,wu2022gnnrecsurvey}。
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.30\textwidth}p{0.62\textwidth}}
        ...
        $r_n$ & 测试样本中真实POI的排序位次 \\
        $\mathcal{L}_{\text{small}},\mathcal{L}_{\text{align}},\mathcal{L}_{\text{llm}}$ & 小模型损失（对应第\ref{chap:method}章$\mathcal{L}_{TSPM}$）、跨空间对齐损失、大模型任务损失（对应$\mathcal{L}_{gen}$） \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}时间先验可用：}训练与测试数据均包含可解析时间戳，且可划分为统一时段；
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}地理信息可用：}POI具备经纬度坐标，允许构建地理约束与距离相关特征；
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}用户历史可观测：}每个测试样本至少存在最小历史长度，以支持序列建模；
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}离线评估优先：}本文主实验聚焦离线Top-$K$评估，不直接讨论在线A/B系统收益。
\end{enumerate}
这些边界并不削弱问题价值，而是用于保证比较公平与可复现。对超出边界的场景（如极短轨迹、无坐标、实时概念漂移），将在第\ref{chap:exp}章“效率与可扩展性分析（RQ5）”中进行工程化讨论，并在第\ref{chap:conclusion}章“研究不足”与“研究展望”中给出扩展路径。
...
\begin{enumerate}
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}Gap-1：} 小模型强结构、弱语义，难以覆盖复杂意图表达与跨场景泛化；
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}Gap-2：} 大模型强语义、弱空间，难以稳定保持地理连续性与可达性约束；
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}Gap-3：} 缺少面向工程部署的统一协同训练方案，难以兼顾效果与成本。
\end{enumerate}

据此提出本文的建模原则：
\begin{enumerate}
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}DP-1（结构先验显式化）：} 在小模型侧显式建模时间异质性与双向转移关系；
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}DP-2（空间语义对齐）：} 在大模型侧引入地理编码与POI结构对齐模块；
    \item {\heiti\addCJKfontfeatures{AutoFakeBold=2}DP-3（协同训练可部署）：} 通过两阶段训练和参数高效微调实现稳定融合。
\end{enumerate}
