...
\section{任务定义与符号约定}
设用户集合为 $\mathcal{U}=\{u_1,u_2,\ldots,u_M\}$，POI集合为 $\mathcal{L}=\{\ell_1,\ell_2,\ldots,\ell_N\}$。用户 $u$ 的第 $i$ 条签到定义为：
\begin{equation}
...
\hphantom{式中：}\symline{$\mathcal{L}$}{候选POI全集。}
...
\subsubsection{显式推理范式：OneRec 与 OneRec-Think 类工作}
